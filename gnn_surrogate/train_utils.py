...
def compute_vector_stats(
    ...
    original_count = len(train_index)
    train_index = select_stat_samples(train_index, max_samples, sampling, seed)
    used_sampling = "all" if len(train_index) == original_count else sampling
    ...
    for path, t in train_index:
        ...
        if value == "velocity":
            ...
            values = sample.target_acceleration[mask].double()
        else:
...
def compute_acceleration_stats(
    ...
    return compute_vector_stats(train_index, cache, "acceleration", max_samples, sampling, seed)


def parse_optional_sample_count(value: str) -> int | None:
    normalized = value.strip().lower()
    if normalized in {"all", "none", "full"}:
        return None
    count = int(value)
    if count <= 0:
        raise ValueError("sample count must be positive, or use 'all'")
    return count


def normalize_output_scale(value: str) -> str:
    if value == "velocity":
        return value
    if value in {"acceleration", "delta"}:
        return "acceleration"
    raise ValueError("output scale must be 'velocity' or 'acceleration'")


def normalize_acceleration_mode(value: str) -> str:
    if value in {"acceleration", "delta"}:
        return "acceleration"
    if value in {"physical_acceleration", "physical"}:
        return "physical_acceleration"
    raise ValueError("acceleration mode must be 'acceleration' or 'physical_acceleration'")
